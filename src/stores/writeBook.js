...
const writeBook = (cardtype, username, password) => {
    ...
    return db.collection("demo-PuncCard").doc("getLastPunch")
    .update({
        data: {
            card_date : moment().format('YYYYMMDD'),
            card_from : "10.10.40.181",
            card_onoff: cardtype,
            card_time : moment().format('HHmm'),
            data_type : cardtype
        }
    })
    .then(() => {
        db.collection("demo-PuncCard").doc("success")
            .get()
            .then((doc) => {
                if (doc.exists){
